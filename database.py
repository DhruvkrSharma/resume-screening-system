...
import pandas as pd
from config import DB_PATH, TOP_N_LIMIT_MAX
...
class ResumeDatabase:
    ...
    def get_results_for_role(self, role_id, top_n=None):
        ...
        if top_n is None:
            ...
            limit_value = int(top_n)
            if limit_value <= 0:
                raise ValueError("top_n must be a positive integer")
            if limit_value > TOP_N_LIMIT_MAX:
                raise ValueError(f"top_n cannot exceed {TOP_N_LIMIT_MAX}")
        ...
        query = """
            ...
            WHERE r.role_id = ?
            ORDER BY r.num_matched_skills DESC, r.similarity_score DESC, r.id ASC
        """
